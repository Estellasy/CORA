...
from datasets.coco_eval import CocoEvaluator, convert_to_xywh   # coco数据集格式转换
from datasets.panoptic_eval import PanopticEvaluator    # PanopticEvaluator 为什么要在全景分割上进行评估
from util.visualizer import COCOVisualizer
from torch.nn.functional import interpolate
from copy import deepcopy
import random
import pycocotools.mask as mask_util    # mask_util有什么作用 哪些数据集是使用mask的
import numpy as np
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        categories = data_loader.dataset.category_list  # 处理类别信息

        # add pseudo labels
        # 处理伪标签 用于半监督学习 CORA+的内容 使用CORA的伪标签
